...
void pareto_set_get_delta(dist_lookup *lookup, mask *msk, double *out_delta, int *p1, int *p2)
{
	*out_delta = DBL_MAX;
	int i, j;
	for (i = 0; i < lookup->npoints; i++)
	{
		...
		for (j = 0; j < i; i++)
		{
			...
			double dist = lookup->distances[i][j];
...
void pareto_set_get_epsilon(dist_lookup *lookup, mask *msk, double *out_epsilon, int *out_p1, int *out_p2)
{
	*out_epsilon = -DBL_MAX;
	int i, j;
	for (i = 0; i < lookup->npoints; i++)
...
dist_lookup *pareto_set_distance_lookup_new(pareto_set *set, double (*metric)(double *p1, double *p2))
{
	...
	for (i = 0; i < set->size; i++)
	{
		ret->distances[i] = (double *) malloc((set->size - i) * sizeof(*ret->distances[i]));
	}
	...
	for (i = 0; i < set->size; i++)
	{
		...
		for (j = 0; j < i; j++)
		{
			ret->distances[i][j] = metric(set->points[i], set->points[j]);
		}
	}
